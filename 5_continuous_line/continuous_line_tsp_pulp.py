...
u = dict()
for i in C:
    u[i] = pulp.LpVariable(cat=pulp.LpInteger, lowBound=1, upBound=num_digits,  name=f'u_{i}')
# add constraints
# exactly one origin
for i in C:
    mdl.addConstraint(pulp.lpSum(x[i, j] for i_, j in x_keys if i_ == i) == 1, name=f'single_origin_{i}')
# exactly one destination
for j in C:
    mdl.addConstraint(pulp.lpSum(x[i, j] for i, j_ in x_keys if j_ == j) == 1, name=f'single_dest_{j}')
# sequence
for i, j in x_keys:
    if j != dummy:
        mdl.addConstraint(u[i] - u[j] + 1 <= (num_digits+1) * (1 - x[i, j]), name=f'seq_{i}_{j}')
# set the objective function
mdl.setObjective(pulp.lpSum(x[i, j] for i, j in x_keys))  # not required for this problem
# endregion
...
u_sol = {(i, j): int(round(u[i, j].value())) for (i, j) in C if (i, j) != dummy}
for i in I:
